...
class OptimizeTradingSystem:
    ...
    def run_trading_system(self, volatility = "rolling", rolling_window=5, lower_threshold=0, upper_threshold = 3, naive=False, verbose=False, plot=False):
        ...
        if volatility != "rolling":
            garch_predictions = pd.read_csv(f"../predictions/validation/predictions/{self.hub1_name}_{self.hub2_name}_v{self.validation_size}_h{self.test_size}_w{self.window_size}_{volatility}_predictions.csv")
    ...
    def study(self, volatility = "rolling", rolling_window_range=[5,60,5], lower_threshold_range=[0,2,0.2], upper_threshold_range=[3,4,0.5], criteria = "profit", min_trades=10, verbose=False, plot=False):
        ...
        if verbose or plot:
            if volatility == "rolling":
                print(f"Best rolling window: {best_rolling_window}, Best lower threshold: {best_lower_threshold}, Best upper threshold: {best_upper_threshold}")
            else:
                print(f"Best lower threshold: {best_lower_threshold:.2f}, Best upper threshold: {best_upper_threshold:.2f}")
            self.run_trading_system(volatility= volatility, rolling_window=best_rolling_window, lower_threshold=best_lower_threshold, upper_threshold=best_upper_threshold, verbose=verbose, plot=plot)
