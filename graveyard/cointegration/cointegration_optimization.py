...
class OptimizeCointegrationTradingSystem:
    def __init__(self, hub1_name, hub2_name, rolling_window, validation_size, test_size, window_size, cointegration_model = "ols", mode = "validation"):
        ...
        self._load_data(cointegration_model)

    def _load_data(self, cointegration_model):
        ...
        cointegration_data = pd.read_csv(f"../predictions/cointegration/{self.hub1_name}_{self.hub2_name}_r{self.rolling_window}_v{self.validation_size}_h{self.test_size}_w{self.window_size}_{cointegration_model}_cointegration.csv")

        self.price_difference = cointegration_data["hub_diff"].values
        self.cointegration_beta = cointegration_data["beta"].values

    def run_trading_system(self, rolling_window=5, lower_threshold=0, upper_threshold = 3, verbose=False, plot=False):

        np.random.seed(42)

        size = self.validations_size if self.mode == "validation" else self.test_size
        ...
        for i in range(size):
            ...
            if (difference <  lower_threshold * difference_std) and (difference >  - upper_threshold * difference_std):
            ...
            elif (difference >  lower_threshold * difference_std) and (difference < difference_mean + upper_threshold * difference_std):
    ...
    def study(self, rolling_window_range=[5,60,5], lower_threshold_range=[0,2,0.2], criteria = "profit", min_trades=10, verbose=False, plot=False):
        best_profit = 0
        ...
        if verbose or plot:
            print(f"Best rolling window: {best_rolling_window:.2f}, Best lower threshold: {best_lower_threshold:.2f}")
            self.run_trading_system(rolling_window=best_rolling_window, lower_threshold=best_lower_threshold, verbose=verbose, plot=plot)
