...
class OptimizeTradingSystem:
    ...
    def _load_data(self):
        ...
        self.returns_difference = pd.DataFrame(hub1_historical_returns - hub2_historical_returns, columns=['difference'])

    def run_trading_system(self, rolling_window=5, lower_threshold=0, upper_treshold = 3, naive=False, verbose=False, plot=False):
        np.random.seed(42)
        ...
        for i in range(self.validation_size):
            ...
            rolling_data = self.returns_difference[["difference"]][-self.test_size - self.validation_size -self.window_size- rolling_window + i: - self.test_size - self.validation_size - self.window_size + i]
            returns_difference_std = rolling_data.std().item()

            r = np.random.rand() 

            if ((returns_difference > lower_threshold * returns_difference_std) and (returns_difference < upper_treshold * returns_difference_std))or (naive and r < 0.5):
                long_position_return = np.log(self.hub1_actuals.values[i].item() / self.hub1_last_available_data.values[i].item())
                short_position_return = - np.log(self.hub2_actuals.values[i].item() / self.hub2_last_available_data.values[i].item())

                long_position_return_with_transaction_costs = np.log((self.hub1_actuals.values[i].item() - 2*transaction_costs) / self.hub1_last_available_data.values[i].item())
                short_position_return_with_transaction_costs = - np.log((self.hub2_actuals.values[i].item() - 2*transaction_costs) / self.hub2_last_available_data.values[i].item())

                net_profit_long = self.hub1_actuals.values[i].item() - self.hub1_last_available_data.values[i].item()
                net_profit_short = self.hub2_last_available_data.values[i].item() - self.hub2_actuals.values[i].item()

                monetary_neutral_adjustment = self.hub1_last_available_data.values[i].item() / self.hub2_last_available_data.values[i].item()

                self.profit += net_profit_long + monetary_neutral_adjustment * net_profit_short - transaction_costs * 4
            ...
            elif ((returns_difference < -lower_threshold * returns_difference_std) and (returns_difference > -upper_treshold * returns_difference_std)) or (naive and r >= 0.5):
                long_position_return = np.log(self.hub2_actuals.values[i].item() / self.hub2_last_available_data.values[i].item())
                short_position_return = - np.log(self.hub1_actuals.values[i].item() / self.hub1_last_available_data.values[i].item())

                long_position_return_with_transaction_costs = np.log((self.hub2_actuals.values[i].item() - 2*transaction_costs) / self.hub2_last_available_data.values[i].item())
                short_position_return_with_transaction_costs = - np.log((self.hub1_actuals.values[i].item() - 2*transaction_costs) / self.hub1_last_available_data.values[i].item())

                net_profit_long = self.hub2_actuals.values[i].item() - self.hub2_last_available_data.values[i].item()
                net_profit_short = self.hub1_last_available_data.values[i].item() - self.hub1_actuals.values[i].item()

                monetary_neutral_adjustment = self.hub2_last_available_data.values[i].item() / self.hub1_last_available_data.values[i].item()
    ...
    def study(self, rolling_window_range=[5,60,5], lower_threshold_range=[0,2,0.2], upper_threshold_range=[3,4,0.5], criteria = "profit", min_trades=10, verbose=False, plot=False):
        ...
        for lower_threshold in np.arange(l_threshold_l, l_threshold_u, l_threshold_s):
            for upper_threshold in np.arange(u_threshold_l, u_threshold_u, u_threshold_s):
                for rolling_window in np.arange(rolling_window_l, rolling_window_u, rolling_window_s):
                        self.run_trading_system(rolling_window=rolling_window, lower_threshold=lower_threshold, upper_treshold=upper_threshold)
        ...
        if verbose or plot:
            ...
            self.run_trading_system(rolling_window=best_rolling_window, lower_threshold=best_lower_threshold, upper_treshold=best_upper_threshold, verbose=verbose, plot=plot)
